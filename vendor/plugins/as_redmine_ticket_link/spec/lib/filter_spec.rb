...
describe AsakusaSatellite::Filter::RedmineTicketLink do
  context "API keyが指定されていない" do
    before do
      config = OpenStruct.new({ :roots => 'http://redmine.example.com/a/'})
      @filter = AsakusaSatellite::Filter::RedmineTicketLink.new(config)
    end

    subject { @filter.process 'foo #223' }
    it { should == 'foo <a target="_blank" href="http://redmine.example.com/a/issues/223">#223</a>' }
  end

  context "API keyが指定されている" do
    before do
      json = <<'JSON'
...
JSON
    ...
    subject { @filter.process('#380') }
    it { should == '<a target="_blank" href="http://redmine.example.com/a/issues/380">#380 Redmineプラグインをタイトルも取れるようにする</a>'}
  end
end
